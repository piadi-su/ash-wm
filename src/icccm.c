...
// Applica le Size Hints (WM_NORMAL_HINTS) rispettando min/max size e resize increments
void ApplyNormalHints(Display *disp, Window w, int *x, int *y, unsigned int *w_out, unsigned int *h_out) {
    ...
    if (XGetWMNormalHints(disp, w, &hints, &supplied)) {
        ...
        // Gestione incrementi di resize (utile per i terminali es. caratteri 8x16)
